...
sslflags	: SESSION CACHE sslcache	{
		...
		| CIPHERS STRING		{
			...
			(void)asprintf(&buf, "tls ciphers \"%s\"", $2);
			proto->opts = opts_add(proto->opts, &proto->optsc, buf);
			free($2);
			free(buf);
		}
		| ECDH CURVE STRING		{
			...
			(void)asprintf(&buf, "tls ecdh curve \"%s\"", $3);
			proto->opts = opts_add(proto->opts, &proto->optsc, buf);
			free($3);
			free(buf);
		}
		| CA FILENAME STRING		{
			...
			(void)asprintf(&buf, "tls ca file \"%s\"", $3);
			proto->opts = opts_add(proto->opts, &proto->optsc, buf);
			free($3);
			free(buf);
		}
		...
		| CA CERTIFICATE STRING		{
			...
			(void)asprintf(&buf, "tls ca cert \"%s\"", $3);
			proto->opts = opts_add(proto->opts, &proto->optsc, buf);
			free($3);
			free(buf);
		}
		| NO flag			{
			char	*buf;
			proto->sslflags &= ~($2);
			(void)asprintf(&buf, "tls no %s", getsslflag($2));
			proto->opts = opts_add(proto->opts, &proto->optsc, buf);
			free(buf);
		}
		| flag				{
			char	*buf;
			proto->sslflags |= $1;
			(void)asprintf(&buf, "tls %s", getsslflag($1));
			proto->opts = opts_add(proto->opts, &proto->optsc, buf);
			free(buf);
		}
		;
...
flag		: STRING			{
			...
			if ($1) {
				yyerror("\"%s\" is deprecated, please remove "
				    "the flag manually", $1);
				free($1);
				YYERROR;
			}
